...
router.get('/user/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [rows] = await db.query<BookingRow[]>(
      'SELECT b.*, r.room_number, h.hotel_name, rt.room_type FROM booking b JOIN rooms r ON b.room_id = r.room_id JOIN hotels h ON r.hotel_id = h.hotel_id JOIN room_types rt ON r.room_type_id = rt.room_type_id WHERE b.user_id = ?',
      [userId]
    );
    res.json(rows);
  } catch (err: any) {
